...
> 架构：9 区域 `url-latency-benchmark` 组 + 25 业务 `static` 组 + ~290 filter_remote + 567 filter_local 规则
...
- QX「策略」面板应看到 34 组（9 url-latency-benchmark + 25 static）
...
2. **策略（Policy）** 面板应出现 34 组（9 `url-latency-benchmark` + 25 `static`）。
