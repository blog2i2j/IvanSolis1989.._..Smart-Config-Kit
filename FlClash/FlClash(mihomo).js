...
// === 导入方法（FlClash，两步操作） ===
//
// ⚠️ 关键：覆写脚本需要先「创建」再「关联」到订阅，只粘贴不关联不会生效！
//
// 【第 1 步：创建覆写脚本】
//   方式 A（URL）：FlClash → 配置 →「覆写脚本」→ 点右上角 + → 填入名称 + 下方 URL：
//     https://raw.githubusercontent.com/IvanSolis1989/Smart-Config-Kit/main/FlClash/FlClash(mihomo).js
//   方式 B（粘贴）：打开上方 GitHub 链接，全选复制 → 粘贴到脚本编辑器
//   方式 C（jsdelivr CDN）：https://cdn.jsdelivr.net/gh/IvanSolis1989/Smart-Config-Kit@main/FlClash/FlClash(mihomo).js
//   保存。
//
// 【第 2 步：关联到订阅】
//   配置 → 点击订阅卡片 ⋮ →「更多」→「覆写」→ 点选刚才创建的覆写脚本 → 确定。
//   回到首页下拉刷新（或重启 FlClash）。
//
// ⚠️ GitHub / jsdelivr 在国内可能被墙，URL 导入前先确保代理已通。
//
