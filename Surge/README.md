...
> 架构：9 区域 url-test 组（include-all-proxies + policy-regex-filter 自动按地区聚合）+ 25 业务策略组 + ~290 RULE-SET
...
### 这是什么？
**Surge 是 iOS / macOS 上最专业、最贵的付费代理客户端**。它的强项是稳定 + 脚本化 + MITM。本仓库提供一份 Surge `.conf`，让你无需手配 900+ 条规则就能享用 9 区域 + 25 业务分流。
...
- Surge「策略组」面板应看到 **34 组**（9 区域 + 25 业务）
