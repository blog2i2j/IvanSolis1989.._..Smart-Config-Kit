...
> 路径 C（Xray 核）产物：`v2rayN/v2rayN(xray).json` v5.2.5-v2n.2（详见 `v2rayN/CHANGELOG.md`）。
...
|---|---|---|---:|---|---|---|
| **A（强烈推荐）** | mihomo (Clash.Meta) | `Clash Meta For Android/CMFA(mihomo).yaml` | 28 业务 select | 9 `url-test` | ❌（YAML 不含 `type: smart`） | ★★★★★ |
| **B** | sing-box | `SingBox/SingBox(sing-box)-full.json` | 28 业务 selector | 9 selector/urltest | ❌（sing-box 无此特性） | ★★★★ |
...
| 文件 | 规则量 | 适合 |
|---|---:|---|
| `SingBox/SingBox(sing-box).json` | 4 rule-sets + 精简 domain_suffix rule | 快速验证 |
| `SingBox/SingBox(sing-box)-full.json`（推荐） | 387 rule-sets + 977 rules | 与 Clash Party 对齐 |
