...
- **想要 25 业务组 + 9 区域组的完整体验** → **路径 A**（mihomo 核）或 **路径 B**（sing-box 核）
...
|---|---|---|---:|---|---|---|
| **A（强烈推荐）** | mihomo (Clash.Meta) | `Clash Meta For Android/CMFA(mihomo).yaml` | 25 业务 select | 9 `url-test` | ❌（YAML 不含 `type: smart`） | ★★★★★ |
| **B** | sing-box | `SingBox/SingBox(sing-box)-full.json` | 25 业务 selector | 9 selector/urltest | ❌（sing-box 无此特性） | ★★★★ |
...
**为什么路径 C 最弱？**
Xray 核心的路由规则只能指向 `proxy / direct / block` 三个出站，无法表达 25 业务 + 9 区域 两层结构，更不能 LightGBM 择优。仅适合不想换核心、只需要基础分流的用户。

---

## 路径 A：mihomo 核心 + Clash YAML（推荐）

这是 v2rayN 路径里**最接近 Clash Party 主线**的方案：25 业务组、9 区域 `url-test` 组、375+ rule-provider 全部原样生效，策略结构和 CMFA 完全一致。
...
适合「想把 v2rayN 当 sing-box 前端」的用户。支持 25 业务组 + 9 区域组，靠 sing-box 原生 `selector` / `urltest` 实现；但没有 LightGBM 自动择优（sing-box 无此特性）。
...
v2rayN 主面板状态栏应显示 sing-box 已启动；打开 `http://127.0.0.1:9090/ui`（若启用了 clash dashboard）可看到 9 区域 + 25 业务组。
...
- ❌ 无 25 业务组 → 9 区域组的两层结构。所有 `proxy` 规则指向同一个节点。
- ❌ 无 LightGBM 自动择优。
