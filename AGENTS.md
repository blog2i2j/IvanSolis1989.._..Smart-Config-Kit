...
另有间接覆盖的客户端不单独出产物：
- **Hiddify**：内核即 sing-box，消费 `SingBox/SingBox(sing-box)*.json`；见 `SingBox/README.md §2a`
...
2. **再同步到其余 9 份产物**：
   ...
   - `OpenClash/OpenClash(mihomo).sh`（Slim）
   - `OpenClash/OpenClash(mihomo-smart).sh`（Full）
...
- `Clash Party/ClashParty(mihomo-smart).js` 中 `SMART` / `BIZ` 常量定义（§4.1 的 37 组命名）
